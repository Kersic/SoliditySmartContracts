...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      this.kycSaleInstance = new this.web3.eth.Contract(
        KycContract.abi,
  ...
  handleKycWhitelisting = async() => {
    await this.kycSaleInstance.methods.setKycCompleted(this.state.kycAddress).send({from: this.accounts[0]});
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        Addres to allow: <input type={"text"} name="kycAddress" value={this.state.kycAddress} onChange={this.handleInputChange} />
        <button type="button" onClick={this.handleKycWhitelisting}>Add to Whitelist</button>

        <h2>Buy Tokens</h2>
        <p>If you want to buy tokens, send Wei to this address: {this.state.tokenSaleAddress}</p>
        <p>You currenty have: {this.state.userTokens}</p>
